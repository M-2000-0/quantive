    ...
    <div className="space-y-6">
          ...
          {alerts.map((alert: any) => (
            <div key={alert.id}
                 className={`bg-zinc-900 rounded-lg border p-4 flex items-start gap-3 ${
                   ...
                   alert.severity === 'warning' ? 'border-amber-800' : 'border-zinc-800'
                 }`}>
              {alert.severity === 'critical' ? <AlertTriangle className="w-5 h-5 text-red-400 mt-0.5" /> :
               alert.severity === 'warning' ? <AlertTriangle className="w-5 h-5 text-amber-400 mt-0.5" /> :
      ...
      <div className="bg-zinc-900 rounded-xl border border-zinc-800 p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div>
            ...
            <p className={`text-xl font-semibold ${analysis.shortfall > 0 ? 'text-amber-400' : 'text-emerald-400'}`}>
        ...
        {analysis.shortfall > 0 && (
          <div className="mt-4 bg-amber-950/30 border border-amber-800 rounded-lg p-4">
      ...
      {uncategorized_txns > 0 && (
        <div className="bg-zinc-900 rounded-lg border border-zinc-800 p-4 flex items-center gap-3">
          ...
          <a href="/personal/transactions" className="text-amber-400 hover:text-amber-300 text-sm flex items-center gap-1">
          ...
          {[
            ...
            <div key={i} className="flex items-center gap-3">
              <div className={`w-2 h-2 rounded-full ${
                ...
                item.status === 'upcoming' ? 'bg-amber-500' : 'bg-zinc-700'
              }`} />
