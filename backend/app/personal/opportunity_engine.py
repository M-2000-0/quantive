...
def _juris_refs(juris: str, candidates: list[str]) -> list[str]:
    """Resolve rule candidates to the user's jurisdiction.

    1. Keep candidates that exist in the user's pack (matching jurisdiction).
    2. Otherwise fall back to the same-topic rule from the user's pack
       (e.g. a US mortgage candidate maps to BD-2026-hra for a BD user).
    3. Otherwise use the GEN fallback with the matching topic.
    4. Never return refs from a foreign jurisdiction — wrong-country tax
       advice is worse than generic advice, so drop the candidate if
       nothing jurisdiction-appropriate exists.
    """
    from app.personal.tax_packs import rules_for

    _by_topic: dict[str, dict] = {}
    # Own jurisdiction first, then GEN as fallback for missing topics.
    for rule in rules_for(juris):
        if rule["jurisdiction"] == juris:
            _by_topic[rule["topic"]] = rule
    for rule in rules_for("GEN"):
        _by_topic.setdefault(rule["topic"], rule)

    def _topic_of(rule_id: str) -> str:
        # e.g. "US-2026-mortgage-interest" -> "mortgage-interest"
        parts = rule_id.split("-")
        return "-".join(parts[2:]) if len(parts) > 3 else rule_id

    def _pack_topics_for(candidate: str) -> list[str]:
        """Map a candidate rule id to topics present in the user's pack."""
        cand_topic = _topic_of(candidate)
        # Direct topic match (candidate topic == pack topic)
        if cand_topic in _by_topic:
            return [cand_topic]
        # Keyword heuristics: map US-specific topics to pack topics
        keyword_map = {
            "mortgage": "housing", "salt": "housing", "state-refund": "housing",
            "rental": "housing", "housing": "housing", "hra": "housing",
            "medical": "health", "health-insurance": "health", "hsa": "health",
            "401k": "retirement", "ira": "retirement", "roth": "retirement",
            "sep": "retirement", "retirement": "retirement", "saver": "retirement",
            "education": "education", "student-loan": "education", "529": "education",
            "educator": "education",
            "home-office": "work", "business-expenses": "work", "vehicle": "work",
            "charitable": "donations", "donations": "donations", "bunching": "donations",
        }
        for kw, topic in keyword_map.items():
            if kw in cand_topic and topic in _by_topic:
                return [topic]
        return []

    resolved: list[str] = []
    for c in candidates:
        for topic in _pack_topics_for(c):
            rule = _by_topic[topic]
            # Only refs from the user's jurisdiction or GEN — never a foreign pack.
            if rule["jurisdiction"] in (juris, "GEN") and rule["id"] not in resolved:
                resolved.append(rule["id"])
    return resolved
...
def detect(db: Session, user_id: str, tax_year: int = 2026, tier: str = "") -> list[PersonalOpportunity]:
    ...
    # ── Jurisdiction pack candidates (BD pack) ───────────────────────
    if juris == "BD" and (has("housing", "rent") or has("housing", "mortgage")
                          or has("employment_detail", "yes") or has("income", "employment_detail", "yes")):
        candidates.append({"title": "House rent allowance", "category": "housing", "relevance": "medium",
                           "why": "Salaried taxpayers may have an exempt portion of house rent allowance under yearly rules.",
                           "needs_info": ["Salaried income", "Actual rent paid"],
                           "needs_docs": ["Rent receipts", "Salary certificate"],
                           "rule_refs": _juris_refs(juris, ["BD-2026-hra"]),
                           "next_action": "Confirm the current exemption formula with NBR rules."})
    ...
    if has("housing", "mortgage") or has("mortgage_detail", "yes"):
        candidates.append({"title": "Mortgage interest", "category": "housing", "relevance": "high",
                           ...
                           "rule_refs": _juris_refs(juris, ["US-2026-mortgage-interest", "GEN-2026-housing-interest"]),
        ...
        if has("tax_regime", "yes") or has("tax_regime", "needs_confirmation"):
            candidates.append({"title": "Mortgage points", "category": "housing", "relevance": "medium",
                               ...
                               "rule_refs": _juris_refs(juris, ["US-2026-mortgage-points"]),
    ...
    if juris == "US" and (has("employment_detail", "yes") or has("housing", "own") or has("housing", "mortgage")):
        candidates.append({"title": "State and local taxes (SALT)", "category": "housing", "relevance": "medium",
                           ...
                           "rule_refs": _juris_refs(juris, ["US-2026-salt"]),
        ...
        candidates.append({"title": "State tax refund recovery", "category": "housing", "relevance": "low",
                           ...
                           "rule_refs": _juris_refs(juris, ["US-2026-state-refund"]),
    ...
    if has("medical_exp", "yes"):
        candidates.append({"title": "Medical expenses (7.5% AGI floor)", "category": "health", "relevance": "medium",
                           ...
                           "rule_refs": _juris_refs(juris, ["US-2026-medical-75pct", "GEN-2026-medical-expenses"]),
    ...
    if has("health_insurance", "yes"):
        if has_any("income_sources", "business", "freelance"):
            candidates.append({"title": "Self-employed health insurance deduction", "category": "health", "relevance": "high",
                               ...
                               "rule_refs": _juris_refs(juris, ["US-2026-se-health-insurance"]),
            ...
            candidates.append({"title": "HSA/FSA optimization", "category": "health", "relevance": "medium",
                               ...
                               "rule_refs": _juris_refs(juris, ["US-2026-hsa", "US-2026-fsa"]),
    ...
    if has("retirement", "yes"):
        candidates.append({"title": "Retirement contributions", "category": "retirement", "relevance": "high",
                           ...
                           "rule_refs": _juris_refs(juris, ["US-2026-401k", "US-2026-traditional-ira", "US-2026-roth-ira"]),
        ...
        if has_any("income_sources", "business", "freelance") or has("filing_status", "single"):
            candidates.append({"title": "Roth conversion ladder", "category": "retirement", "relevance": "medium",
                               ...
                               "rule_refs": _juris_refs(juris, ["US-2026-roth-conversion"]),
        ...
        if has_any("income_sources", "business", "freelance"):
            candidates.append({"title": "SEP-IRA contributions", "category": "retirement", "relevance": "high",
                               ...
                               "rule_refs": _juris_refs(juris, ["US-2026-sep-ira"]),
        ...
        candidates.append({"title": "Retirement Savings Credit", "category": "retirement", "relevance": "low",
                           ...
                           "rule_refs": _juris_refs(juris, ["US-2026-education-credit-saver"]),
    ...
    if has("education_exp", "yes"):
        if juris == "US" and has("loans", "yes"):
            candidates.append({"title": "Student loan interest", "category": "education", "relevance": "medium",
                               ...
                               "rule_refs": _juris_refs(juris, ["US-2026-student-loan-interest"]),
            ...
            candidates.append({"title": "Education expenses", "category": "education", "relevance": "low",
                               ...
                               "rule_refs": _juris_refs(juris, ["US-2026-american-opportunity", "US-2026-lifetime-learning", "GEN-2026-education"]),
    ...
    if has_any("family", "children", "dependents") or has("dependents_count", "1") or has("dependents_count", "2") or has("dependents_count", "3+"):
        candidates.append({"title": "529 Education Savings Plan", "category": "education", "relevance": "medium",
                           ...
                           "rule_refs": _juris_refs(juris, ["US-2026-529-plan"]),
    ...
    if has("employment_detail", "yes"):
        candidates.append({"title": "Educator expense deduction", "category": "education", "relevance": "low",
                           ...
                           "rule_refs": _juris_refs(juris, ["US-2026-educator-expenses"]),
    ...
    if has("home_office", "yes"):
        candidates.append({"title": "Home office deduction", "category": "work", "relevance": "high",
                           ...
                           "rule_refs": _juris_refs(juris, ["US-2026-home-office", "GEN-2026-home-office"]),
    ...
    if has("work_expenses", "yes"):
        candidates.append({"title": "Business expense deduction", "category": "work", "relevance": "medium",
                           ...
                           "rule_refs": _juris_refs(juris, ["US-2026-business-expenses"]),
    ...
    if has("work_expenses", "yes"):
        candidates.append({"title": "Vehicle expense deduction", "category": "work", "relevance": "low",
                           ...
                           "rule_refs": _juris_refs(juris, ["US-2026-vehicle-expenses"]),
    ...
    if has_any("family", "children", "dependents"):
        candidates.append({"title": "Child Tax Credit", "category": "family", "relevance": "high",
                           ...
                           "rule_refs": _juris_refs(juris, ["US-2026-child-tax-credit"]),
        ...
        candidates.append({"title": "Child and Dependent Care Credit", "category": "family", "relevance": "medium",
                           ...
                           "rule_refs": _juris_refs(juris, ["US-2026-child-care-credit", "US-2026-dependent-care-fsa"]),
    ...
    if has("dependents_count", "1") or has("dependents_count", "2") or has("dependents_count", "3+"):
        ...
        candidates.append({"title": "Dependent Care FSA", "category": "family", "relevance": "medium",
                           ...
                           "rule_refs": _juris_refs(juris, ["US-2026-dependent-care-fsa"]),
    ...
    if has("donations", "yes"):
        candidates.append({"title": "Charitable donations", "category": "donations", "relevance": "medium" if juris == "US" else "low",
                           ...
                           "rule_refs": _juris_refs(juris, ["US-2026-charitable", "GEN-2026-donations"]),
        ...
        candidates.append({"title": "Charitable contribution bunching", "category": "donations", "relevance": "medium",
                           ...
                           "rule_refs": _juris_refs(juris, ["US-2026-charitable-bunching"]),
    ...
    if has("invest_accounts", "yes") or has("investment_income", "yes"):
        candidates.append({"title": "Tax-loss harvesting", "category": "investments", "relevance": "high",
                           ...
                           "rule_refs": _juris_refs(juris, ["US-2026-tax-loss-harvesting"]),
        ...
        candidates.append({"title": "Long-term capital gains optimization", "category": "investments", "relevance": "medium",
                           ...
                           "rule_refs": _juris_refs(juris, ["US-2026-capital-gains-rates"]),
        ...
        candidates.append({"title": "Foreign Tax Credit", "category": "investments", "relevance": "low",
                           ...
                           "rule_refs": _juris_refs(juris, ["US-2026-foreign-tax-credit"]),
    ...
    if has_any("income_sources", "business", "freelance"):
        candidates.append({"title": "Self-employment tax deduction", "category": "business", "relevance": "high",
                           ...
                           "rule_refs": _juris_refs(juris, ["US-2026-se-tax-deduction"]),
        ...
        candidates.append({"title": "QBI deduction (Section 199A)", "category": "business", "relevance": "high",
                           ...
                           "rule_refs": _juris_refs(juris, ["US-2026-qualified-business-income"]),
        ...
        candidates.append({"title": "Section 179 expensing", "category": "business", "relevance": "medium",
                           ...
                           "rule_refs": _juris_refs(juris, ["US-2026-section-179"]),
        ...
        candidates.append({"title": "Startup costs deduction", "category": "business", "relevance": "low",
                           ...
                           "rule_refs": _juris_refs(juris, ["US-2026-startup-costs"]),
        ...
        candidates.append({"title": "R&D Tax Credit", "category": "business", "relevance": "low",
                           ...
                           "rule_refs": _juris_refs(juris, ["US-2026-rd-credit"]),
    ...
    if has("rental_income", "yes"):
        if juris == "US":
            candidates.append({"title": "Rental property expenses", "category": "business", "relevance": "high",
                               ...
                               "rule_refs": _juris_refs(juris, ["US-2026-rental-expenses"]),
            ...
            candidates.append({"title": "Rental income deductions", "category": "housing", "relevance": "medium",
                               ...
                               "rule_refs": _juris_refs(juris, ["BD-2026-rental-income"]),
    ...
    if has_any("housing", "own", "mortgage"):
        candidates.append({"title": "Energy efficiency home improvements", "category": "energy", "relevance": "low",
                           ...
                           "rule_refs": _juris_refs(juris, ["US-2026-energy-efficient-home"]),
        ...
        candidates.append({"title": "Solar/清洁能源 credit", "category": "energy", "relevance": "low",
                           ...
                           "rule_refs": _juris_refs(juris, ["US-2026-solar-credit"]),
    ...
    if has_any("income_sources", "business", "freelance"):
        candidates.append({"title": "Estimated tax payments", "category": "compliance", "relevance": "high",
                           ...
                           "rule_refs": _juris_refs(juris, ["US-2026-estimated-tax", "US-2026-safe-harbor"]),
    ...
    if has("invest_accounts", "yes"):
        candidates.append({"title": "Net Investment Income Tax awareness", "category": "investments", "relevance": "low",
                           ...
                           "rule_refs": _juris_refs(juris, ["US-2026-net-investment-income"]),
